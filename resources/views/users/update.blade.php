...
@section('content')
    <div class="container my-5" style="max-width: 600px;">
        ...
        {{-- نموذج الإضافة --}}
        <form action="{{ route('users.update', $users->id) }}" enctype="multipart/form-data" method="POST" novalidate
            ...
            <div class="mb-3 text-center position-relative" style="width: 200px; margin: auto;">
                {{-- صورة المستخدم --}}
                <img id="profileImage" src="{{ asset('storage/' . $users->image) }}" alt="صورة المستخدم"
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    ...
                    <input type="text" id="name" name="name"
                        class="form-control @error('name') is-invalid @enderror" value="{{ old('name') ?? $users->name }}"
                        required autofocus>
                    @error('name')
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    ...
                    <input type="text" id="location" name="location"
                        ...
                        value="{{ old('location') ?? $users->location }}" required autofocus>
                    @error('location')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            {{-- الدور --}}
            @if (Auth::check() && Auth::user()->hasRole('Content Managment'))
                <div class="mb-3">
                    ...
                    <select id="role-id" name="role-id" class="form-select @error('role-id') is-invalid @enderror">
                        @foreach ($roles as $role)
                            <option value="{{ $role->id }}" @selected($role->id === $current_role->id)>{{ $role->name }} </option>
                        @endforeach
                    </select>
                    @error('role-id')
                        <div class="invalid-feedback">{{ $message }}</div>
